...
public class EdgeCrossingTrimRouterTests
{
    ...
    [Test]
    public void TrySelectUniqueChordPairing_SelectsSingleValidPair()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        var active = new List<EdgeCrossingTrimRouter.LocalCrossing>
        {
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=0, edgeStart=0, edgeEnd=1, t=0.2f, isBeforeInside=true},
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=0, edgeStart=0, edgeEnd=1, t=0.8f, isBeforeInside=false},
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=1, edgeStart=1, edgeEnd=2, t=0.3f, isBeforeInside=true},
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=2, edgeStart=2, edgeEnd=0, t=0.6f, isBeforeInside=false},
        };
        bool ok = EdgeCrossingTrimRouter.TrySelectUniqueChordPairing(ctx, active, out var c0, out var c1, out var reason);
        Assert.IsTrue(ok, reason);
        Assert.AreNotEqual(c0.a.edgeIndex, c0.b.edgeIndex);
        Assert.AreNotEqual(c1.a.edgeIndex, c1.b.edgeIndex);
    }

    [Test]
    public void TryBuildInsideBoundarySegments_MismatchBeforeInside_Fails()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        ctx.SampleInside = _ => false;
        var edgeInfos = new List<EdgeCrossingTrimRouter.EdgeInfo>
        {
            new EdgeCrossingTrimRouter.EdgeInfo
            {
                edgeIndex = 0,
                edgeStart = 0,
                edgeEnd = 1,
                crossings = new List<EdgeCrossingTrimRouter.LocalCrossing>
                {
                    new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=0, edgeStart=0, edgeEnd=1, t=0.5f, isBeforeInside=true }
                }
            }
        };
    ...
    [Test]
    public void TrySelectUniqueChordPairing_CountNotFour_Fails()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        var active = new List<EdgeCrossingTrimRouter.LocalCrossing>
        {
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=0, edgeStart=0, edgeEnd=1, t=0.2f, isBeforeInside=true},
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=1, edgeStart=1, edgeEnd=2, t=0.3f, isBeforeInside=true},
            new EdgeCrossingTrimRouter.LocalCrossing{ edgeIndex=2, edgeStart=2, edgeEnd=0, t=0.4f, isBeforeInside=true},
        };
        bool ok = EdgeCrossingTrimRouter.TrySelectUniqueChordPairing(ctx, active, out var _, out var _, out var reason);
        Assert.IsFalse(ok);
        Assert.AreEqual("active_crossing_count_not_4", reason);
    }

    [Test]
    public void TryExtractInsideLoops_ClosedCycle_Succeeds()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        var c0 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 0, edgeStart = 0, edgeEnd = 1, t = 0.2f, isBeforeInside = false };
        var c1 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 1, edgeStart = 1, edgeEnd = 2, t = 0.6f, isBeforeInside = true };
        var c2 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 2, edgeStart = 2, edgeEnd = 0, t = 0.4f, isBeforeInside = false };
        var c3 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 0, edgeStart = 0, edgeEnd = 1, t = 0.8f, isBeforeInside = true };
        var boundary = new List<EdgeCrossingTrimRouter.LoopSegment>
        {
            new EdgeCrossingTrimRouter.LoopSegment(EdgeCrossingTrimRouter.MakeCrossingNode(c0), EdgeCrossingTrimRouter.MakeCrossingNode(c1)),
            new EdgeCrossingTrimRouter.LoopSegment(EdgeCrossingTrimRouter.MakeCrossingNode(c2), EdgeCrossingTrimRouter.MakeCrossingNode(c3)),
        };
        bool ok = EdgeCrossingTrimRouter.TryExtractInsideLoops(
            ...
            out var reason);
        Assert.IsTrue(ok, reason);
        Assert.AreEqual(1, loops.Length);
        Assert.GreaterOrEqual(loops[0].Length, 3);
    }

    [Test]
    public void TryExtractInsideLoops_DegreeOneNode_Fails()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        var c0 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 0, edgeStart = 0, edgeEnd = 1, t = 0.2f, isBeforeInside = false };
        var c1 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 1, edgeStart = 1, edgeEnd = 2, t = 0.6f, isBeforeInside = true };
        ...
        bool ok = EdgeCrossingTrimRouter.TryExtractInsideLoops(
            ...
            out var reason);
        Assert.IsFalse(ok);
        StringAssert.Contains("degree", reason);
    }

    [Test]
    public void TryExtractInsideLoops_DuplicateUv_Fails()
    {
        var ctx = MakeContext(new Dictionary<EdgeCrossingTrimRouter.EdgeKey, List<EdgeCrossingTrimRouter.EdgeCrossing>>());
        var c0 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 0, edgeStart = 0, edgeEnd = 1, t = 0.2f, isBeforeInside = false };
        var c1 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 0, edgeStart = 0, edgeEnd = 1, t = 0.2f, isBeforeInside = true }; // same UV as c0
        var c2 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 1, edgeStart = 1, edgeEnd = 2, t = 0.6f, isBeforeInside = false };
        var c3 = new EdgeCrossingTrimRouter.LocalCrossing { edgeIndex = 2, edgeStart = 2, edgeEnd = 0, t = 0.4f, isBeforeInside = true };
        var boundary = new List<EdgeCrossingTrimRouter.LoopSegment>
        {
            new EdgeCrossingTrimRouter.LoopSegment(EdgeCrossingTrimRouter.MakeCrossingNode(c0), EdgeCrossingTrimRouter.MakeCrossingNode(c2)),
            new EdgeCrossingTrimRouter.LoopSegment(EdgeCrossingTrimRouter.MakeCrossingNode(c3), EdgeCrossingTrimRouter.MakeCrossingNode(c1)),
        };
        bool ok = EdgeCrossingTrimRouter.TryExtractInsideLoops(
            ctx,
            boundary,
            ...
            out var reason);
        Assert.IsFalse(ok);
        StringAssert.Contains("invalid_chord", reason);
    }
